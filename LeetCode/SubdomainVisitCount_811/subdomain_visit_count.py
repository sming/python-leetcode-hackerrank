...
foreach sdp
    split via space
        sd1, sd2, sd3 = sd.split('.')
            c.update(sd2)
            c.update(sd3)

l = list()
foreach count_pair in c.items():
    l.Append(f"{count_pair[1] {count_pair[2]}")
return
"""
...
class Solution:
    ...
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for count_domain in cpdomains:
            count_str, domain = count_domain.split(" ")
            ...
            sub_domains: List[str] = domain.split(".")

            if len(sub_domains) > 2:
                counts[sub_domains[1] + "." + sub_domains[2]] += count

            counts[sub_domains[-1]] += count

        result: List[str] = list()
        for tuple in counts.items():
            result.append(f"{tuple[1]} {tuple[0]}")
